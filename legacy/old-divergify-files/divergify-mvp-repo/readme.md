...
## One-command tunnel workflow
If you use Expo Go on a different network segment you probably run three terminals (backend, Cloudflare tunnel, Expo). After installing [cloudflared](https://developers.cloudflare.com/cloudflare-one/connections/connect-apps/install-and-setup/installation/), you can now launch everything from a single tab:
...
- ensure the Express stub has its dependencies installed the first time
- start the backend on port 8787
- open a Cloudflared tunnel and grab the public HTTPS URL
- launch `expo start --tunnel` with `EXPO_PUBLIC_BACKEND_URL` pointed at that tunnel

Press `Ctrl+C` once to stop all three processes. Check the console output for the tunnel URL if you want to share it or need to override it manually.
